...
export default function ResendVerificationPage() {
    ...
    const resendMutation = useMutation<BaseResponse, AxiosError<IApiErrorResponse>, ResendFormValues>({
        mutationFn: async (data) => {
            const response = await axios.post(`${API_BASE_URL}/api/auth/resend-verification`, data);
            return response.data;
        },
        onSuccess: (data) => {
            toast.success("Link Terkirim!", {
                description: data.message || "Silakan cek kotak masuk email Anda.",
                icon: <CheckCircle2 className="text-emerald-500" />,
            });
            // Arahkan kembali ke halaman info verifikasi
        ...
        >
            ...
            <div className="hidden lg:flex w-[60%] p-6 relative flex-col justify-center">
                <div className="w-full h-full rounded-[2.5rem] bg-[#111318] relative overflow-hidden border border-white/5 shadow-2xl flex flex-col p-12 justify-between">
                    ...
                    {/* Illustration/Widget Replacement */}
